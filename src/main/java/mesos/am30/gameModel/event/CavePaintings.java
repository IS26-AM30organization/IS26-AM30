...
public class CavePaintings implements IF_Event {
     ...
     *       (player.tribe(ARTIST).size() > artistMinimum ==> player.parameters(PRESTIGE_POINTS) = \old(player.parameters(PRESTIGE_POINTS)) + (gainedPrestigePoints * player.tribe(ARTIST).size()))
     *
